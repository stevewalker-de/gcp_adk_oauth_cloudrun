...
from config import COOKIE_MANAGER_SECRET, GCP_PROJECT
...
def get_cookie_manager(prefix: str = "knock_knock_app/"):
    ...
    try:
        ...
        if not password:
            st.warning(
                f"⚠️ Could not retrieve secret '{COOKIE_MANAGER_SECRET}' from Secret Manager and COOKIE_PASSWORD env var is not set. Using a fallback for local dev."
            )
